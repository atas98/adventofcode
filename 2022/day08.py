...
input_file = open('./2022/day08_input_test.txt')
forrest_map = [[int(h) for h in line.rstrip()] for line in input_file.readlines() if line]

POSITIONS = ('N', 'S', 'W', 'E')


@lru_cache
def is_visible(x, y, from_pos, value):
    if from_pos == 'N':
        if y == 0:
            return True
        return is_visible(x, y - 1, from_pos, value) and forrest_map[y][x] < value
    if from_pos == 'S':
        if y == len(forrest_map) - 1:
            return True
        return is_visible(x, y + 1, from_pos, value) and forrest_map[y][x] < value
    if from_pos == 'W':
        if x == 0:
            return True
        return is_visible(x - 1, y, from_pos, value) and forrest_map[y][x] < value
    if from_pos == 'E':
        if x == len(forrest_map[0]) - 1:  # raises exception if not forrest_map
            return True
        return is_visible(x + 1, y, from_pos, value) and forrest_map[y][x] < value
...
print(
    '\n'.join([
        ''.join([
            # str(forrest_map[y][x])
            str(int(any(is_visible(x, y, pos, val) for pos in POSITIONS)))
            for x, val in enumerate(forrest_map[y])
        ]) for y, val in enumerate(forrest_map)
    ])
)
print(sum(
            any(is_visible(x, y, pos, val) for pos in POSITIONS)
                for y, val in enumerate(forrest_map)
                for x, val in enumerate(forrest_map[y])
))
